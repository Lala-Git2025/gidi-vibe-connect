    ...
    <div className={`py-4 flex items-center gap-4 ${isExpired ? 'opacity-60' : ''}`}>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          <span className={`flex-shrink-0 text-xs px-2 py-0.5 rounded-full ${isExpired ? 'bg-muted text-muted-foreground' : 'bg-amber-500 text-white'}`}>
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-amber-500">{loading ? '—' : active.length}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2">
