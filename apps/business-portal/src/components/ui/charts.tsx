...
export function Sparkline({
  ...
  return (
    <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
      {area && areaPath && <path d={areaPath} fill={color} fillOpacity={0.16} />}
      <path
        d={path}
        fill="none"
        stroke={color}
...
export function AreaChart({
  ...
  const linePath = (c: [number, number][]) =>
    c.map(([x, y], i) => (i === 0 ? `M${x},${y}` : `L${x},${y}`)).join(' ');
  const areaPath = (c: [number, number][]) =>
    `${linePath(c)} L${width},${height} L0,${height} Z`;

  return (
    <div style={{ position: 'relative' }}>
      <svg width="100%" viewBox={`0 0 ${width} ${height + 24}`} preserveAspectRatio="none">
        <defs>
          <linearGradient id="bp2-area-fill" x1="0" y1="0" x2="0" y2="1">
        ...
        <path d={areaPath(mainCoords)} fill="url(#bp2-area-fill)" />
        <path
          d={linePath(mainCoords)}
          fill="none"
          stroke={color}
          strokeWidth="2.5"
          strokeLinecap="round"
...
export function BarChart({ data, color = '#EAB308' }: BarChartProps) {
  if (!data.length) return null;
...
export function DonutChart({ data, size = 140, thickness = 18 }: DonutChartProps) {
