  ...
  try {
    ...
    try {
      ...
      const scrapeResponse = await supabase.functions.invoke('scrape-lagos-venues', {
        body: { category, lga: lga || 'Lagos Island' }
      });
      ...
      console.log('⚠️ Web scraping failed, trying Google Places:', scrapeError instanceof Error ? scrapeError.message : 'Unknown error');
    }
